        ...
        public bool IsValid
        {
            get
            {
                ...
                if (string.IsNullOrWhiteSpace(Value))
                    throw new AppException(CarObjectErrors.CarNameIsRequired);

                if (Value.Length < 5)
                    throw new AppException(CarObjectErrors.CarNameIsInvalidTooSmall);

                if (Value.Length > 50)
                    throw new AppException(CarObjectErrors.CarNameIsInvalidTooLarge);

                if (Value.Any(char.IsDigit))
                    throw new AppException(CarObjectErrors.CarNameIsInvalid);
